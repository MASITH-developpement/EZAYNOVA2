/* global odoo */
odoo.define('eazynova_website.website', function (require) {
    'use strict';
    ...
    publicWidget.registry.EazynovaPriceCalculator = publicWidget.Widget.extend({
        ...
        _updatePrice: function () {
            var nbUsers = parseInt($('#nb_users_calc').val()) || 5;
            var basePrice = 250;
            var includedUsers = 5;
            var extraUserPrice = 20;
            var setupFee = 1800;

            var extraUsers = Math.max(0, nbUsers - includedUsers);
            var extraPrice = extraUsers * extraUserPrice;
            var totalMonthly = basePrice + extraPrice;
        ...
        _updateFormPrice: function () {
            var planSelect = $('#plan_id');
            var nbUsersInput = $('#nb_users');

            if (!planSelect.length || !nbUsersInput.length) return;

            var selectedOption = planSelect.find('option:selected');
            var basePrice = parseFloat(selectedOption.data('monthly-price')) || 250;
            var includedUsers = parseInt(selectedOption.data('included-users')) || 5;
            var extraUserPrice = parseFloat(selectedOption.data('extra-user-price')) || 20;
            var setupFee = parseFloat(selectedOption.data('setup-fee')) || 1800;
            var nbUsers = parseInt(nbUsersInput.val()) || 5;
    ...
    publicWidget.registry.EazynovaSignupForm = publicWidget.Widget.extend({
        ...
        _onSubmit: function (ev) {
            ...
            var termsCheckbox = form.querySelector('#terms');
